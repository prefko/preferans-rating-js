...
const _validatePlayer = (p) => {
	if (!_.isPlainObject(p)) {
		throw new Error("PrefRating::_validatePlayer:Invalid player: " + p);
	} else if (!_.isString(p.username)) {
		throw new Error("PrefRating::_validatePlayer:Missing username: " + p);
	} else if (!_.isNumber(p.score)) {
		throw new Error("PrefRating::_validatePlayer:Missing username: " + p);
	} else if ((!_.isNumber(p.oldRating) && !_.isNumber(p.rating))) {
		throw new Error("PrefRating::_validatePlayer:Missing original rating: " + p);
	}
};
