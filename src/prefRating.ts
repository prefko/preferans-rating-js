...
const _d = (p1: PrefRatingPlayer, p2: PrefRatingPlayer, bula: number): number =>
	Number(math.round(
		math
			.chain(p1.score)
			.subtract(p2.score)
...
const _calculateDs = (p1: PrefRatingPlayer, p2: PrefRatingPlayer, p3: PrefRatingPlayer, bula: number): { D12: number, D13: number, D23: number } => ({
	D12: _d(p1, p2, bula),
	D13: _d(p1, p3, bula),
	D23: _d(p2, p3, bula),
});

const _t = (p1: PrefRatingPlayer, p2: PrefRatingPlayer): number =>
	Number(math.round(
		math
			.chain(p2.rating)
			.subtract(p1.rating)
...
const _calculateTs = (p1: PrefRatingPlayer, p2: PrefRatingPlayer, p3: PrefRatingPlayer): { T12: number, T13: number, T23: number } => ({
	T12: _t(p1, p2),
	T13: _t(p1, p3),
	T23: _t(p2, p3),
});
...
const _calculateNs = (p1: PrefRatingPlayer, p2: PrefRatingPlayer, p3: PrefRatingPlayer, bula: number): { N12: number, N13: number, N23: number } => ({
	N12: _n(p1, p2, bula),
	N13: _n(p1, p3, bula),
	N23: _n(p2, p3, bula),
});

const _c = (c1: number, c2: number): number =>
	Number(math.round(
		math
			.chain(c1)
			.add(c2)
...
const _calculateChanges = (p1: PrefRatingPlayer, p2: PrefRatingPlayer, p3: PrefRatingPlayer, bula: number): { change1: number, change2: number, change3: number } => {
	const D: { D12: number, D13: number, D23: number } = _calculateDs(p1, p2, p3, bula);
	const T: { T12: number, T13: number, T23: number } = _calculateTs(p1, p2, p3);
	const N: { N12: number, N13: number, N23: number } = _calculateNs(p1, p2, p3, bula);
